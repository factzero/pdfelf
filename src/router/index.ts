import { createRouter, createWebHistory } from 'vue-router'

const router = createRouter({
  history: createWebHistory(),
  scrollBehavior() {
    return { top: 0 }
  },
  routes: [
    {
      path: '/',
      alias: '/en',
      name: 'home',
      component: () => import('@/pages/HomePage.vue'),
      meta: { titleKey: 'seo.homeTitle', descKey: 'seo.homeDesc', homePage: true },
    },
    // 整理
    {
      path: '/compress-pdf',
      alias: '/en/compress-pdf',
      name: 'compress-pdf',
      component: () => import('@/pages/CompressPage.vue'),
      meta: { titleKey: 'seo.compressPdf', descKey: 'tools.compressPdf.desc' },
    },
    {
      path: '/merge-pdf',
      alias: '/en/merge-pdf',
      name: 'merge-pdf',
      component: () => import('@/pages/MergePage.vue'),
      meta: { titleKey: 'seo.mergePdf', descKey: 'tools.mergePdf.desc' },
    },
    {
      path: '/split-pdf',
      alias: '/en/split-pdf',
      name: 'split-pdf',
      component: () => import('@/pages/SplitPage.vue'),
      meta: { titleKey: 'seo.splitPdf', descKey: 'tools.splitPdf.desc' },
    },
    {
      path: '/rotate-pdf',
      alias: '/en/rotate-pdf',
      name: 'rotate-pdf',
      component: () => import('@/pages/RotatePage.vue'),
      meta: { titleKey: 'seo.rotatePdf', descKey: 'tools.rotatePdf.desc' },
    },
    {
      path: '/delete-pages',
      alias: '/en/delete-pages',
      name: 'delete-pages',
      component: () => import('@/pages/DeletePage.vue'),
      meta: { titleKey: 'seo.deletePages', descKey: 'tools.deletePages.desc' },
    },
    {
      path: '/extract-pages',
      alias: '/en/extract-pages',
      name: 'extract-pages',
      component: () => import('@/pages/ExtractPage.vue'),
      meta: { titleKey: 'seo.extractPages', descKey: 'tools.extractPages.desc' },
    },
    {
      path: '/reorder-pages',
      alias: '/en/reorder-pages',
      name: 'reorder-pages',
      component: () => import('@/pages/PdfReorderPage.vue'),
      meta: { titleKey: 'seo.reorderPages', descKey: 'tools.reorderPages.desc' },
    },
    // 编辑
    {
      path: '/add-watermark',
      alias: '/en/add-watermark',
      name: 'add-watermark',
      component: () => import('@/pages/WatermarkPage.vue'),
      meta: { titleKey: 'seo.addWatermark', descKey: 'tools.addWatermark.desc' },
    },
    {
      path: '/add-page-numbers',
      alias: '/en/add-page-numbers',
      name: 'add-page-numbers',
      component: () => import('@/pages/AddPageNumberPage.vue'),
      meta: { titleKey: 'seo.addPageNumbers', descKey: 'tools.addPageNumbers.desc' },
    },
    {
      path: '/add-header-footer',
      alias: '/en/add-header-footer',
      name: 'add-header-footer',
      component: () => import('@/pages/AddHeaderFooterPage.vue'),
      meta: { titleKey: 'seo.headerFooter', descKey: 'tools.headerFooter.desc' },
    },
    // PDF 对比
    {
      path: '/compare-pdf',
      alias: '/en/compare-pdf',
      name: 'compare-pdf',
      component: () => import('@/pages/ComparePdfPage.vue'),
      meta: { titleKey: 'seo.comparePdf', descKey: 'tools.comparePdf.desc' },
    },
    // 安全
    {
      path: '/protect-pdf',
      alias: '/en/protect-pdf',
      name: 'protect-pdf',
      component: () => import('@/pages/ProtectPdfPage.vue'),
      meta: { titleKey: 'seo.protectPdf', descKey: 'tools.protectPdf.desc' },
    },
    {
      path: '/unlock-pdf',
      alias: '/en/unlock-pdf',
      name: 'unlock-pdf',
      component: () => import('@/pages/UnlockPdfPage.vue'),
      meta: { titleKey: 'seo.unlockPdf', descKey: 'tools.unlockPdf.desc' },
    },
    {
      path: '/crop-pdf',
      alias: '/en/crop-pdf',
      name: 'crop-pdf',
      component: () => import('@/pages/CropPdfPage.vue'),
      meta: { titleKey: 'seo.cropPdf', descKey: 'tools.cropPdf.desc' },
    },
    {
      path: '/extract-images',
      alias: '/en/extract-images',
      name: 'extract-images',
      component: () => import('@/pages/ExtractImagesPage.vue'),
      meta: { titleKey: 'seo.extractImages', descKey: 'tools.extractImages.desc' },
    },
    {
      path: '/repair-pdf',
      alias: '/en/repair-pdf',
      name: 'repair-pdf',
      component: () => import('@/pages/RepairPdfPage.vue'),
      meta: { titleKey: 'seo.repairPdf', descKey: 'tools.repairPdf.desc' },
    },
    // 从 PDF 转换
    {
      path: '/pdf-to-word',
      alias: '/en/pdf-to-word',
      name: 'pdf-to-word',
      component: () => import('@/pages/PdfToWordPage.vue'),
      meta: { titleKey: 'seo.pdfToWord', descKey: 'tools.pdfToWord.desc' },
    },
    {
      path: '/pdf-to-excel',
      alias: '/en/pdf-to-excel',
      name: 'pdf-to-excel',
      component: () => import('@/pages/PdfToExcelPage.vue'),
      meta: { titleKey: 'seo.pdfToExcel', descKey: 'tools.pdfToExcel.desc' },
    },
    {
      path: '/pdf-to-ppt',
      alias: '/en/pdf-to-ppt',
      name: 'pdf-to-ppt',
      component: () => import('@/pages/PdfToPptPage.vue'),
      meta: { titleKey: 'seo.pdfToPpt', descKey: 'tools.pdfToPpt.desc' },
    },
    {
      path: '/pdf-to-image',
      alias: '/en/pdf-to-image',
      name: 'pdf-to-image',
      component: () => import('@/pages/PdfToImagePage.vue'),
      meta: { titleKey: 'seo.pdfToImage', descKey: 'tools.pdfToImage.desc' },
    },
    {
      path: '/pdf-to-jpg',
      alias: '/en/pdf-to-jpg',
      name: 'pdf-to-jpg',
      component: () => import('@/pages/PdfToJpgPage.vue'),
      meta: { titleKey: 'seo.pdfToJpg', descKey: 'tools.pdfToJpg.desc' },
    },
    {
      path: '/pdf-to-png',
      alias: '/en/pdf-to-png',
      name: 'pdf-to-png',
      component: () => import('@/pages/PdfToPngPage.vue'),
      meta: { titleKey: 'seo.pdfToPng', descKey: 'tools.pdfToPng.desc' },
    },
    {
      path: '/pdf-to-tiff',
      alias: '/en/pdf-to-tiff',
      name: 'pdf-to-tiff',
      component: () => import('@/pages/PdfToTiffPage.vue'),
      meta: { titleKey: 'seo.pdfToTiff', descKey: 'tools.pdfToTiff.desc' },
    },
    {
      path: '/pdf-to-svg',
      alias: '/en/pdf-to-svg',
      name: 'pdf-to-svg',
      component: () => import('@/pages/PdfToSvgPage.vue'),
      meta: { titleKey: 'seo.pdfToSvg', descKey: 'tools.pdfToSvg.desc' },
    },
    {
      path: '/pdf-to-text',
      alias: '/en/pdf-to-text',
      name: 'pdf-to-text',
      component: () => import('@/pages/PdfToTextPage.vue'),
      meta: { titleKey: 'seo.pdfToText', descKey: 'tools.pdfToText.desc' },
    },
    {
      path: '/pdf-to-html',
      alias: '/en/pdf-to-html',
      name: 'pdf-to-html',
      component: () => import('@/pages/PdfToHtmlPage.vue'),
      meta: { titleKey: 'seo.pdfToHtml', descKey: 'tools.pdfToHtml.desc' },
    },
    // 转换成 PDF
    {
      path: '/word-to-pdf',
      alias: '/en/word-to-pdf',
      name: 'word-to-pdf',
      component: () => import('@/pages/WordToPdfPage.vue'),
      meta: { titleKey: 'seo.wordToPdf', descKey: 'tools.wordToPdf.desc' },
    },
    {
      path: '/excel-to-pdf',
      alias: '/en/excel-to-pdf',
      name: 'excel-to-pdf',
      component: () => import('@/pages/ExcelToPdfPage.vue'),
      meta: { titleKey: 'seo.excelToPdf', descKey: 'tools.excelToPdf.desc' },
    },
    {
      path: '/ppt-to-pdf',
      alias: '/en/ppt-to-pdf',
      name: 'ppt-to-pdf',
      component: () => import('@/pages/PptToPdfPage.vue'),
      meta: { titleKey: 'seo.pptToPdf', descKey: 'tools.pptToPdf.desc' },
    },
    {
      path: '/image-to-pdf',
      alias: '/en/image-to-pdf',
      name: 'image-to-pdf',
      component: () => import('@/pages/ImageToPdfPage.vue'),
      meta: { titleKey: 'seo.imageToPdf', descKey: 'tools.imageToPdf.desc' },
    },
    {
      path: '/html-to-pdf',
      alias: '/en/html-to-pdf',
      name: 'html-to-pdf',
      component: () => import('@/pages/HtmlToPdfPage.vue'),
      meta: { titleKey: 'seo.htmlToPdf', descKey: 'tools.htmlToPdf.desc' },
    },
    // 阅读器
    {
      path: '/pdf-reader',
      alias: '/en/pdf-reader',
      name: 'pdf-reader',
      component: () => import('@/pages/PdfReaderPage.vue'),
      meta: { titleKey: 'seo.pdfReader', descKey: 'tools.pdfReader.desc' },
    },
    // 更多工具
    {
      path: '/ocr-pdf',
      alias: '/en/ocr-pdf',
      name: 'ocr-pdf',
      component: () => import('@/pages/OcrPdfPage.vue'),
      meta: { titleKey: 'seo.ocrPdf', descKey: 'tools.ocrPdf.desc' },
    },
    {
      path: '/edit-pdf',
      alias: '/en/edit-pdf',
      name: 'edit-pdf',
      component: () => import('@/pages/EditPdfPage.vue'),
      meta: { titleKey: 'seo.editPdf', descKey: 'tools.editPdf.desc' },
    },
    {
      path: '/edit-metadata',
      alias: '/en/edit-metadata',
      name: 'edit-metadata',
      component: () => import('@/pages/EditMetadataPage.vue'),
      meta: { titleKey: 'seo.editMetadata', descKey: 'tools.editMetadata.desc' },
    },
    {
      path: '/flip-pdf',
      alias: '/en/flip-pdf',
      name: 'flip-pdf',
      component: () => import('@/pages/FlipPdfPage.vue'),
      meta: { titleKey: 'seo.flipPdf', descKey: 'tools.flipPdf.desc' },
    },
    {
      path: '/grayscale-pdf',
      alias: '/en/grayscale-pdf',
      name: 'grayscale-pdf',
      component: () => import('@/pages/GrayscalePdfPage.vue'),
      meta: { titleKey: 'seo.grayscalePdf', descKey: 'tools.grayscalePdf.desc' },
    },
    {
      path: '/resize-pdf',
      alias: '/en/resize-pdf',
      name: 'resize-pdf',
      component: () => import('@/pages/ResizePdfPage.vue'),
      meta: { titleKey: 'seo.resizePdf', descKey: 'tools.resizePdf.desc' },
    },
    {
      path: '/sign-pdf',
      alias: '/en/sign-pdf',
      name: 'sign-pdf',
      component: () => import('@/pages/SignPage.vue'),
      meta: { titleKey: 'seo.signPdf', descKey: 'tools.signPdf.desc' },
    },
    {
      path: '/redact-pdf',
      alias: '/en/redact-pdf',
      name: 'redact-pdf',
      component: () => import('@/pages/RedactPdfPage.vue'),
      meta: { titleKey: 'seo.redactPdf', descKey: 'tools.redactPdf.desc' },
    },
    {
      path: '/fill-form',
      alias: '/en/fill-form',
      name: 'fill-form',
      component: () => import('@/pages/FillFormPage.vue'),
      meta: { titleKey: 'seo.fillForm', descKey: 'tools.fillForm.desc' },
    },
    // Phase 3 扩展工具
    {
      path: '/pdf-to-pdfa',
      alias: '/en/pdf-to-pdfa',
      name: 'pdf-to-pdfa',
      component: () => import('@/pages/PdfaPage.vue'),
      meta: { titleKey: 'seo.pdfToPdfa', descKey: 'tools.pdfToPdfa.desc' },
    },
    {
      path: '/flatten-pdf',
      alias: '/en/flatten-pdf',
      name: 'flatten-pdf',
      component: () => import('@/pages/FlattenPdfPage.vue'),
      meta: { titleKey: 'seo.flattenPdf', descKey: 'tools.flattenPdf.desc' },
    },
    {
      path: '/epub-to-pdf',
      alias: '/en/epub-to-pdf',
      name: 'epub-to-pdf',
      component: () => import('@/pages/EpubToPdfPage.vue'),
      meta: { titleKey: 'seo.epubToPdf', descKey: 'tools.epubToPdf.desc' },
    },
    {
      path: '/txt-to-pdf',
      alias: '/en/txt-to-pdf',
      name: 'txt-to-pdf',
      component: () => import('@/pages/TxtToPdfPage.vue'),
      meta: { titleKey: 'seo.txtToPdf', descKey: 'tools.txtToPdf.desc' },
    },
    {
      path: '/pdf-overlay',
      alias: '/en/pdf-overlay',
      name: 'pdf-overlay',
      component: () => import('@/pages/PdfOverlayPage.vue'),
      meta: { titleKey: 'seo.pdfOverlay', descKey: 'tools.pdfOverlay.desc' },
    },
    {
      path: '/nup-pdf',
      alias: '/en/nup-pdf',
      name: 'nup-pdf',
      component: () => import('@/pages/NupPage.vue'),
      meta: { titleKey: 'seo.nupPdf', descKey: 'tools.nupPdf.desc' },
    },
    {
      path: '/split-by-bookmarks',
      alias: '/en/split-by-bookmarks',
      name: 'split-by-bookmarks',
      component: () => import('@/pages/BookmarkSplitPage.vue'),
      meta: { titleKey: 'seo.bookmarkSplit', descKey: 'tools.bookmarkSplit.desc' },
    },
    {
      path: '/deskew-pdf',
      alias: '/en/deskew-pdf',
      name: 'deskew-pdf',
      component: () => import('@/pages/DeskewPage.vue'),
      meta: { titleKey: 'seo.deskewPdf', descKey: 'tools.deskewPdf.desc' },
    },
    {
      path: '/bates-numbering',
      alias: '/en/bates-numbering',
      name: 'bates-numbering',
      component: () => import('@/pages/BatesPage.vue'),
      meta: { titleKey: 'seo.batesNumbering', descKey: 'tools.batesNumbering.desc' },
    },
  ],
})

export default router
