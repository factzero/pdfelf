<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Globe :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToHtml.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfToHtml.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToHtml.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToHtml.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToHtml.after') }}</span>
...
import { pdfToHtml, htmlToBlob } from '@/services/htmlService'
...
async function convert() {
  ...
  store.startProcessing(t('pdfToHtml.converting'))
  try {
    const result = await pdfToHtml(selectedFile.value, (p) => store.updateProgress(p))
    const blob = htmlToBlob(result.html)
    resultBlob.value = blob
    outputFilename.value = generateOutputFilename(selectedFile.value.name, 'html', 'html')
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfToHtml.failed'))
    errorMsg.value = t('pdfToHtml.failed')
  }
}
</script>
