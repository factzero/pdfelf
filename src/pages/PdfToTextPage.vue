<template>
  <div class="tool-page container">
    <h1 class="tool-title"><FileType :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToText.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfToText.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToText.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToText.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToText.after') }}</span>
...
import { pdfToText, textToBlob } from '@/services/textService'
...
async function convert() {
  ...
  store.startProcessing(t('pdfToText.converting'))
  try {
    const result = await pdfToText(selectedFile.value, (p) => store.updateProgress(p))
    const blob = textToBlob(result.text)
    resultBlob.value = blob
    outputFilename.value = generateOutputFilename(selectedFile.value.name, 'txt', 'text')
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfToText.failed'))
    errorMsg.value = t('pdfToText.failed')
  }
}
</script>
