<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Presentation :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToPpt.title') }}</h1>
    ...
    <div v-if="selectedFile" class="file-preview">
      ...
      <p class="file-preview__hint">{{ $t('pdfToPpt.renderNote') }}</p>
    </div>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToPpt.completed') }}</p>
...
import { FileText, Presentation } from 'lucide-vue-next'
...
import { pdfToPpt, ensureWorker } from '@/services/wordService'
...
async function convert() {
  ...
  store.startProcessing(t('pdfToPpt.stages.preparing'))
  try {
    const blob = await pdfToPpt(
      selectedFile.value,
      (info) => {
        const text = t(`pdfToPpt.stages.${info.stage}`, info.params ?? {})
    ...
    console.error('[PdfToPpt]', msg, e)
    store.setError(msg)
    errorMsg.value = msg || t('pdfToPpt.failed')
  }
}
</script>
