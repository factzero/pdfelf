<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Bookmark :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('bookmarkSplit.title') }}</h1>
    <p class="tool-desc">{{ $t('bookmarkSplit.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      <template v-if="!isProcessing && !resultBlob">
        <p class="action-card__note">{{ $t('bookmarkSplit.note') }}</p>
        <button class="btn btn--primary btn--large" @click="doSplit">
          {{ $t('bookmarkSplit.splitBtn') }}
        </button>
      </template>
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('bookmarkSplit.completed') }}</p>
    ...
    <ToolSeoContent :ns="'bookmarkSplit'" />
  </div>
</template>
...
import { FileText, Bookmark } from 'lucide-vue-next'
...
import { splitByBookmarks } from '@/services/bookmarkSplitService'
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = selectedFile.value.name.replace(/\.pdf$/i, '-bookmarks.zip')
...
async function doSplit() {
  ...
  store.startProcessing(t('bookmarkSplit.splitting'))
  try {
    const result = await splitByBookmarks(selectedFile.value, (p) => store.updateProgress(p))
    resultBlob.value = result.blob
    outputFilename.value = result.fileName
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('bookmarkSplit.failed'))
    errorMsg.value = t('bookmarkSplit.failed')
  }
}
