  ...
  <div class="tool-page container">
    ...
    <div v-if="selectedFile" class="action-card">
      <button
        ...
        :disabled="toExtract.length === 0"
        @click="extract"
      >
        {{ $t('extract.extractBtn', { n: toExtract.length }) }}
      </button>
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('extract.completed') }}</p>
...
function selectAll() {
  toExtract.value = Array.from({ length: pageCount.value }, (_, i) => i + 1)
}
...
async function extract() {
  ...
  try {
    ...
    store.setError(msg)
    errorMsg.value = msg || t('extract.failed')
  }
}
</script>
