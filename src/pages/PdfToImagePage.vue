<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Image :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToImage.title') }}</h1>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToImage.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToImage.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToImage.after') }}</span>
...
const resultSize = computed(() => {
  ...
  return formatFileSize(resultBlob.value.size)
})
...
.result-compare__value {
  font-weight: 600;
  font-size: 0.9375rem;
}

.result-compare__value--highlight {
  color: var(--color-primary);
}

.result-compare__arrow {
  color: var(--color-text-muted);
  font-size: 1.25rem;
}
