<template>
  <div class="tool-page container">
    <h1 class="tool-title"><FileImage :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToTiff.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfToTiff.desc') }}</p>
    ...
    <div v-if="selectedFile" class="options">
      <div class="option-group">
        <label class="option-label">{{ $t('pdfToTiff.resolution') }}</label>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToTiff.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToTiff.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToTiff.after') }}</span>
...
async function convert() {
  ...
  store.startProcessing(t('pdfToTiff.converting'))
  try {
    const result = await pdfToImage(selectedFile.value, 'tiff', dpi.value, (p) => store.updateProgress(p))
    resultBlob.value = result.blob
    outputFilename.value = result.isSingle
      ? generateOutputFilename(selectedFile.value.name, 'tiff', 'tiff')
      : generateOutputFilename(selectedFile.value.name, 'zip', 'tiff')
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfToTiff.failed'))
    errorMsg.value = t('pdfToTiff.failed')
  }
}
</script>
