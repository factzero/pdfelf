<template>
  <div class="tool-page container">
    <h1 class="tool-title"><FileCheck :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfa.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfa.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfa.completed') }}</p>
...
import { FileText, FileCheck } from 'lucide-vue-next'
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(selectedFile.value.name, 'pdfa')
...
async function doConvert() {
  ...
  store.startProcessing(t('pdfa.converting'))
  try {
    const blob = await convertToPdfA(selectedFile.value, (p) => store.updateProgress(p))
    resultBlob.value = blob
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfa.failed'))
    errorMsg.value = t('pdfa.failed')
  }
}
