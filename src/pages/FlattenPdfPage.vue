<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Layers :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('flatten.title') }}</h1>
    <p class="tool-desc">{{ $t('flatten.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      <template v-if="!isProcessing && !resultBlob">
        <p class="action-card__note">{{ $t('flatten.note') }}</p>
        <button class="btn btn--primary btn--large" @click="doFlatten">
          {{ $t('flatten.flattenBtn') }}
        </button>
      </template>
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('flatten.completed') }}</p>
...
import { flattenPdf } from '@/services/flattenService'
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(selectedFile.value.name, 'flattened')
...
async function doFlatten() {
  ...
  store.startProcessing(t('flatten.flattening'))
  try {
    const blob = await flattenPdf(selectedFile.value, (p) => store.updateProgress(p))
    resultBlob.value = blob
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('flatten.failed'))
    errorMsg.value = t('flatten.failed')
  }
}
