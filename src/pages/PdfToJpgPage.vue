<template>
  <div class="tool-page container">
    <h1 class="tool-title"><FileImage :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToJpg.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfToJpg.desc') }}</p>
    ...
    <div v-if="selectedFile" class="options">
      <div class="option-group">
        <label class="option-label">{{ $t('pdfToJpg.resolution') }}</label>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToJpg.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToJpg.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToJpg.after') }}</span>
...
async function convert() {
  ...
  store.startProcessing(t('pdfToJpg.converting'))
  try {
    const result = await pdfToImage(selectedFile.value, 'jpeg', dpi.value, (p) => store.updateProgress(p))
    resultBlob.value = result.blob
    outputFilename.value = result.isSingle
      ? generateOutputFilename(selectedFile.value.name, 'jpg', 'jpg')
      : generateOutputFilename(selectedFile.value.name, 'zip', 'jpg')
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfToJpg.failed'))
    errorMsg.value = t('pdfToJpg.failed')
  }
}
</script>
