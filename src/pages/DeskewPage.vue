<template>
  <div class="tool-page container">
    <h1 class="tool-title"><AlignCenter :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('deskew.title') }}</h1>
    <p class="tool-desc">{{ $t('deskew.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      <template v-if="!isProcessing && !resultBlob">
        <p class="action-card__note">{{ $t('deskew.note') }}</p>
        <button class="btn btn--primary btn--large" @click="doDeskew">
          {{ $t('deskew.deskewBtn') }}
        </button>
      </template>
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('deskew.completed') }}</p>
...
import { FileText, AlignCenter } from 'lucide-vue-next'
...
import { deskewPdf } from '@/services/deskewService'
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(selectedFile.value.name, 'deskewed')
...
async function doDeskew() {
  ...
  store.startProcessing(t('deskew.deskewing'))
  try {
    const blob = await deskewPdf(selectedFile.value, (p) => store.updateProgress(p))
    resultBlob.value = blob
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('deskew.failed'))
    errorMsg.value = t('deskew.failed')
  }
}
