<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Hash :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('bates.title') }}</h1>
    <p class="tool-desc">{{ $t('bates.desc') }}</p>
    ...
    <div v-if="selectedFile && !isProcessing && !resultBlob" class="options">
      <div class="options__group">
        <div class="option-row">
          <label class="option-label">{{ $t('bates.prefix') }}</label>
          <input v-model="options.prefix" class="option-input" :placeholder="$t('bates.prefixPlaceholder')" />
        </div>
        <div class="option-row">
          <label class="option-label">{{ $t('bates.startNumber') }}</label>
          <input v-model.number="options.startNumber" type="number" min="1" class="option-input" />
        </div>
        <div class="option-row">
          <label class="option-label">{{ $t('bates.digits') }}</label>
          <select v-model.number="options.digits" class="option-select">
        ...
        <div class="option-row">
          <label class="option-label">{{ $t('bates.fontSize') }}</label>
        ...
        <div class="option-row">
          <label class="option-label">{{ $t('bates.position') }}</label>
          <select v-model="options.position" class="option-select">
            <option value="bottom-right">{{ $t('bates.posBottomRight') }}</option>
            <option value="bottom-left">{{ $t('bates.posBottomLeft') }}</option>
            <option value="bottom-center">{{ $t('bates.posBottomCenter') }}</option>
            <option value="top-right">{{ $t('bates.posTopRight') }}</option>
            <option value="top-left">{{ $t('bates.posTopLeft') }}</option>
            <option value="top-center">{{ $t('bates.posTopCenter') }}</option>
          </select>
        </div>
        <div class="option-row option-row--checkbox">
          <input type="checkbox" v-model="options.includePageCount" id="inc-pc" />
          <label for="inc-pc">{{ $t('bates.includePageCount') }}</label>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('bates.completed') }}</p>
...
import { generateOutputFilename, readFileAsArrayBuffer, formatFileSize, downloadBlob } from '@/utils/fileUtils'
import { pdfjsLib, DEFAULT_PDF_OPTIONS } from '@/utils/pdfjs'
import { addBatesNumbering, type BatesOptions } from '@/services/batesService'
...
const options = reactive<BatesOptions>({
  prefix: 'BATES-',
  startNumber: 1,
  digits: 6,
  fontSize: 10,
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(selectedFile.value.name, 'bates')
...
async function doBates() {
  ...
  store.startProcessing(t('bates.numbering'))
  try {
    const blob = await addBatesNumbering(
      selectedFile.value,
      {
        ...
        includePageCount: options.includePageCount,
      },
    ...
    store.setError(e instanceof Error ? e.message : t('bates.failed'))
    errorMsg.value = t('bates.failed')
  }
}
