<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Palette :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('grayscalePdf.title') }}</h1>
    <p class="tool-desc">{{ $t('grayscalePdf.desc') }}</p>
    ...
    <div v-if="selectedFile && !isProcessing && !resultBlob" class="action-card">
      <button class="btn btn--primary btn--large" @click="doGrayscale">
        {{ $t('grayscalePdf.convertBtn') }}
      </button>
    </div>
    ...
    <div v-if="selectedFile && resultBlob" class="action-card">
      <div class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('grayscalePdf.completed') }}</p>
    ...
    <ToolSeoContent ns="grayscalePdf" />
  </div>
</template>
...
import { grayscalePdf } from '@/services/pdfService'
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(f.name, 'pdf', 'grayscale')
...
async function doGrayscale() {
  ...
  errorMsg.value = ''
  store.startProcessing(t('grayscalePdf.converting'))
  try {
    resultBlob.value = await grayscalePdf(selectedFile.value, (p) => store.updateProgress(p))
    store.finishProcessing()
  } catch (e: any) {
    store.setError(e.message || t('grayscalePdf.failed'))
    errorMsg.value = e.message || t('grayscalePdf.failed')
  }
}
...
.result-file-size { font-size: 0.875rem; color: var(--color-text); font-weight: 600; margin-top: var(--spacing-sm); margin-bottom: 0; }
