<template>
  <div class="tool-page container">
    <h1 class="tool-title"><BookOpen :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('epubToPdf.title') }}</h1>
    <p class="tool-desc">{{ $t('epubToPdf.desc') }}</p>
    ...
    <div v-if="selectedFile" class="file-preview">
      <div class="file-preview__thumbnail" @mouseenter="showDelete = true" @mouseleave="showDelete = false">
        <div class="file-preview__placeholder">
          <BookOpen :size="48" :stroke-width="1" class="file-preview__placeholder-icon" />
        </div>
    ...
    <div v-if="selectedFile" class="action-card">
      <template v-if="!isProcessing && !resultBlob">
        <p class="action-card__note">{{ $t('epubToPdf.note') }}</p>
        <button class="btn btn--primary btn--large" @click="doConvert">
          {{ $t('epubToPdf.convertBtn') }}
        </button>
      </template>
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('epubToPdf.completed') }}</p>
...
import { convertEpubToPdf } from '@/services/epubToPdfService'
...
function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = selectedFile.value.name.replace(/\.epub$/i, '.pdf')
}

async function doConvert() {
  if (!selectedFile.value) return
  store.startProcessing(t('epubToPdf.converting'))
  try {
    const blob = await convertEpubToPdf(selectedFile.value, (p) => store.updateProgress(p))
    resultBlob.value = blob
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('epubToPdf.failed'))
    errorMsg.value = t('epubToPdf.failed')
  }
}
