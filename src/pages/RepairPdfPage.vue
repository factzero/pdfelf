<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Wrench :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('repair.title') }}</h1>
    <p class="tool-desc">{{ $t('repair.desc') }}</p>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('repair.completed') }}</p>
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(f.name, 'pdf', 'repaired')
  ...
  try {
    ...
    // Toast error if render fails — the file may be damaged
  }
}
...
async function doRepair() {
  ...
  errorMsg.value = ''
  store.startProcessing(t('repair.repairing'))
  try {
    resultBlob.value = await repairPDF(selectedFile.value, (p) => store.updateProgress(p))
    store.finishProcessing()
  } catch (e: any) {
    store.setError(e.message || t('repair.failed'))
    errorMsg.value = e.message || t('repair.failed')
  }
}
