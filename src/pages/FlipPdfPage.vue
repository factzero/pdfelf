<template>
  <div class="tool-page container">
    ...
    <div v-if="selectedFile && !isProcessing && !resultBlob" class="action-card">
      <div class="flip-options">
        <label class="flip-option" :class="{ 'flip-option--active': flipMode === 'horizontal' }">
          <input type="radio" v-model="flipMode" value="horizontal" />
          <span class="flip-option__icon">↔️</span>
          <span class="flip-option__text">{{ $t('flipPdf.horizontal') }}</span>
        </label>
        <label class="flip-option" :class="{ 'flip-option--active': flipMode === 'vertical' }">
          <input type="radio" v-model="flipMode" value="vertical" />
          <span class="flip-option__icon">↕️</span>
          <span class="flip-option__text">{{ $t('flipPdf.vertical') }}</span>
        </label>
      </div>
      <button class="btn btn--primary btn--large" @click="doFlip">
        {{ $t('flipPdf.flipBtn') }}
      </button>
    </div>
    ...
    <div v-if="selectedFile && resultBlob" class="action-card">
      <div class="action-card__result">
        <div class="result-icon">✅</div>
        <div class="result-body">
          <p class="result-title">{{ $t('flipPdf.completed') }}</p>
...
import { flipPdf } from '@/services/pdfService'
...
const flipMode = ref<'horizontal' | 'vertical'>('horizontal')
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(f.name, 'pdf', 'flipped')
...
async function doFlip() {
  ...
  errorMsg.value = ''
  store.startProcessing(t('flipPdf.flipping'))
  try {
    resultBlob.value = await flipPdf(selectedFile.value, flipMode.value, (p) => store.updateProgress(p))
    store.finishProcessing()
  } catch (e: any) {
    store.setError(e.message || t('flipPdf.failed'))
    errorMsg.value = e.message || t('flipPdf.failed')
  }
}
...
.flip-options { display: flex; gap: var(--spacing-md); margin-bottom: var(--spacing-lg); }
.flip-option { flex: 1; display: flex; flex-direction: column; align-items: center; gap: var(--spacing-sm); padding: var(--spacing-lg); border: 2px solid var(--color-border); border-radius: var(--radius-lg); cursor: pointer; transition: border-color 0.2s; }
.flip-option input { display: none; }
.flip-option--active { border-color: var(--color-primary); background: var(--color-primary-light); }
.flip-option__icon { font-size: 1.5rem; }
.flip-option__text { font-weight: 600; }
...
.result-filename { font-size: 0.8125rem; color: var(--color-text-muted); word-break: break-all; margin-bottom: 0; }
