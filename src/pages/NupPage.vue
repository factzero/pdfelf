<template>
  <div class="tool-page container">
    <h1 class="tool-title"><LayoutGrid :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('nup.title') }}</h1>
    <p class="tool-desc">{{ $t('nup.desc') }}</p>
    ...
    <div v-if="selectedFile && !isProcessing && !resultBlob" class="options">
      <div class="option-row">
        <label class="option-label">{{ $t('nup.layout') }}</label>
        <select v-model="layout" class="option-select">
          <option value="2x1">{{ $t('nup.layout2x1') }}</option>
          <option value="2x2">{{ $t('nup.layout2x2') }}</option>
          <option value="3x2">{{ $t('nup.layout3x2') }}</option>
          <option value="3x3">{{ $t('nup.layout3x3') }}</option>
          <option value="4x2">{{ $t('nup.layout4x2') }}</option>
          <option value="4x4">{{ $t('nup.layout4x4') }}</option>
        </select>
      </div>
    </div>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('nup.completed') }}</p>
...
import { createNupPdf, type NupLayout } from '@/services/nupService'
...
const layout = ref<NupLayout>('2x2')
...
async function onFileSelected(file: File | File[]) {
  ...
  outputFilename.value = generateOutputFilename(selectedFile.value.name, 'nup')
...
async function doNup() {
  if (!selectedFile.value) return
  store.startProcessing(t('nup.processing'))
  try {
    const blob = await createNupPdf(selectedFile.value, layout.value, (p) => store.updateProgress(p))
    resultBlob.value = blob
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('nup.failed'))
    errorMsg.value = t('nup.failed')
  }
}
