<template>
  <div class="tool-page container">
    <h1 class="tool-title"><Maximize :size="28" :stroke-width="2" class="tool-title__icon" /> {{ $t('pdfToSvg.title') }}</h1>
    <p class="tool-desc">{{ $t('pdfToSvg.desc') }}</p>
    ...
    <div v-if="selectedFile" class="options">
      <div class="option-group">
        <label class="option-label">{{ $t('pdfToSvg.resolution') }}</label>
    ...
    <div v-if="selectedFile" class="action-card">
      ...
      <div v-if="resultBlob" class="action-card__result">
        ...
        <div class="result-body">
          <p class="result-title">{{ $t('pdfToSvg.completed') }}</p>
          <div class="result-compare">
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToSvg.before') }}</span>
              <span class="result-compare__value">{{ originalSize }}</span>
            </span>
            <span class="result-compare__arrow">→</span>
            <span class="result-compare__item">
              <span class="result-compare__label">{{ $t('pdfToSvg.after') }}</span>
...
import { pdfToSvg, svgToBlob } from '@/services/svgService'
...
async function convert() {
  ...
  store.startProcessing(t('pdfToSvg.converting'))
  try {
    const result = await pdfToSvg(selectedFile.value, dpi.value, (p) => store.updateProgress(p))
    const blob = svgToBlob(result.svg)
    resultBlob.value = blob
    outputFilename.value = generateOutputFilename(selectedFile.value.name, 'svg', 'svg')
    store.finishProcessing()
  } catch (e) {
    store.setError(e instanceof Error ? e.message : t('pdfToSvg.failed'))
    errorMsg.value = t('pdfToSvg.failed')
  }
}
</script>
