...
免费在线 PDF 处理工具，提供 **38 种** PDF 工具，所有处理在浏览器本地完成，无需上传服务器。
...
### 💬 其他
- ✅ 📣 **用户反馈** — 首页右下角浮动反馈按钮，支持 Bug 报告/功能建议/其他反馈

目前共 **40 个工具**，全部免费、不限次数。
...
| 任务 | 说明 | 方案 | 难度 | 状态 |
|---|---|---|---|---|
| **OCR 文字识别** | 扫描件/图片 PDF → 提取文字 | Tesseract.js (WASM)，浏览器本地运行 | ⭐⭐⭐ | ✅ |
| **编辑 PDF 内容** | 在 PDF 页面上添加文字、图片、遮盖 | pdf-lib + pdfjs 交互式双层 Canvas | ⭐⭐⭐⭐ | ✅ |
| **PDF 对比** | 并排对比两个 PDF 的文本差异 | pdf.js 文本层 diff + 高亮 | ⭐⭐ | ⬜ |
...
| 任务 | 说明 |
|---|---|
| **PDF 转 PDF/A** | 转为长期存档格式 |
| **拼合 PDF (Flatten)** | 将可填写表单转为只读 |
| **EPUB 转 PDF** | 电子书格式转换 |
| **TXT 转 PDF** | 纯文本文件转 PDF |
| **PDF 叠加 (Overlay)** | 两个 PDF 图层叠加合并 |
| **N-up 排版** | 多页内容拼到一页纸上 |
| **按书签拆分** | 根据 PDF 目录自动拆分 |
| **纠偏** | 自动校正扫描件倾斜角度 |
| **Bates 编号** | 法律文件批号印章 |
